...
- `useIssues` selects `sla_target_hours, sla_breached_at, first_response_at, created_at, resolved_at` and `slaState(issue: SlaIssueFields, now?: Date): SlaState` lives in `src/lib/slaState.ts` (R4a contracts; the issues CSV and the issue evidence pack use both).
- `src/components/reports/fortress/CoverageGrid.tsx` renders the per-type coverage grid with `rows: { buildingId, buildingName, cells: Record<ReportType, { status, reportId? }> }[]` — R4a's plan was still being written when this plan was pinned, so the coverage-grid file name and the `Settings.tsx` tab layout are the two names to confirm from R4a's plan/Status before Tasks 4 and 5 start (adapt the one import in Task 5 and the one `TabsTrigger` in Task 4; nothing else depends on them).
- R4a raises `issue_sla_breached` from SQL (`mark_sla_breaches()`), so it never passes through `createNotifications` and cannot push; R4a defers "push for the assignee" to R4b. It stays deferred here too — a push needs the edge-function path, and this slice adds none for SLA. Record it in the Status follow-ups.
