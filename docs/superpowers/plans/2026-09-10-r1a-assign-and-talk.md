...
- [ ] Regenerate `types.ts`, then drop the `as never` / narrow-cast notes in `useBuildingMembers.ts`, `IssueCommentComposer.tsx`, `ResolveIssueDialog.tsx`, `ChecklistsTab.tsx`.
- [ ] Push both repos.
