...
| `compliance_scores` view | `create or replace view … with (security_invoker = on)`: the 2026-06-19_02 body (incl. the `ohs_stated_pct` override) plus `and r.response is not null` in the per_group WHERE; columns `(assessment_id, report_id, building_id, compliance_pct)` unchanged |
| `compliance_section_scores` view | same treatment of the 2026-06-13_08 body: `where r.response is not null`; columns `(assessment_id, building_id, section_no, section_title, section_pct)` unchanged |
| `compliance_critical_scores` view | same: `where r.response is not null`; columns `(assessment_id, building_id, critical_pct)` unchanged |
...
**Score views (added in review, 2026-09-12):** the migration also re-creates `compliance_scores` (2026-06-19_02:10-32), `compliance_section_scores` and `compliance_critical_scores` (2026-06-13_08:63-71, :75-83). All three divided `count(*) filter (where response in ('yes','na'))` by a count with no null filter, so the comment-only rows Task 2/3 start writing scored as FAILS until answered. Each `create or replace view` keeps the column list, order and types, the grants (a replace leaves the ACL alone) and the Yarona `reports.meta->>'ohs_stated_pct'` override verbatim, adds `and r.response is not null`, and restates `with (security_invoker = on)` — `create or replace view` REPLACES storage options with whatever the statement names, which is how 2026-06-19_02 lost the option (A-01, fixed 2026-08-04_01). Local check: one scored item `'yes'` + one scored item `null` + comment → 100.0, not 50.0; `reloptions` still `{security_invoker=on}` on all three. The vendored file is the source of truth for the exact text.
