...
| 8 | Admin operations (deactivate / role change / reassignment) | ✅ | ✅ | ✅ `admin-ops-smoke.mjs` (14, prod) | n/a | this file | **READY (verified)** — role change live (no refresh), deactivate cuts access immediately + blocks new login + audited, self-guard holds. **F-35 built**: "Edit building access" reassigns user_buildings. **Delete user built**: `delete-user` edge fn (admin-guarded, self + last-admin guards) + typed-email confirm dialog → clean re-add path. NB: edge fns only on prod, not staging |
