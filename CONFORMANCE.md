...
| A5 | — | Min-length 8 everywhere, but no zxcvbn meter or HIBP check on password-set surfaces |
...
| A11 | — | No guard unit tests yet (16 CI test files exist, none cover ProtectedRoute) |
...
| E6 | partial | 16 vitest files green in CI; live smoke suites (`npm run smoke`) complement; guard-unit + invite→accept→login CI tests still missing |
